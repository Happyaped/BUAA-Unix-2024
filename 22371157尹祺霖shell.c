...
int main() {
	...
	while (1) {
		...
		if (count != 0) {
			...
			while (cnt < count) {
				if (pipe(fd + 2 * cnt) < 0) {		
					fprintf(stderr, "Xuan's Shell Error: Fail to create the pipe\n");
					exit(1);}
				cnt++;	}
			pid_t pid;
			// 管道输入，标准输出
			...
			pid = fork();
			if (pid < 0) {
				fprintf(stderr, "Xuan's Shell Error: Fail to fork\n");
				exit(1);
			}
			if (pid == 0) {
...
void printPath() {
	...
	fprintf(stdout, "Xuan's Shell %s$ ", path);
	fflush(stdout); // 清空缓冲区 
}
		...
		if (command[i] == ' ') {
			if (j != 0) {
				...
				if (flag == 1) { // 重定向输入
					...
					if (fd < 0) {
						fprintf(stderr, "Xuan's Shell Error: Fail to open the file: '%s'\n", argv[argc]);}
					dup2(fd, STDIN_FILENO);
					fdCnt++;
				} else if (flag == 2) { // 重定向输出
					int fd = open(argv[argc], O_WRONLY | O_CREAT | O_TRUNC, 0x777);
					if (fd < 0) {
						fprintf(stderr, "Xuan's Shell Error: Fail to create the file: '%s'\n", argv[argc]);}
	...
	if (strcmp(argv[0], "type") == 0) { // 'type'指令，识别一个指令是内部指令还是外部指令 
		...
		while (ins[i] != NULL) {
			if (strcmp(ins[i], argv[1]) == 0) {
				fprintf(stdout, "%s is a Xuan's Shell builtin\n", argv[1]);
				return;
			}
			i++;}
		fprintf(stdout, "%s is not a Xuan's Shell builtin\n", argv[1]);
		...
		if (chdir(argv[1]) != 0) { // 使用'chdir' 
			fprintf(stderr, "Xuan's Shell Error: Fail to change directory to: %s\n", argv[1]);
		} else {
			fprintf(stdout, "Xuan's Shell: Successfully change the directory!\n");	}
	} else if (strcmp(argv[0], "pwd") == 0) { // 'pwd'指令
		char *path = (char *)malloc(105 * sizeof(char));
		if (getcwd(path, 105 * sizeof(char)) == NULL) { // 使用'getcwd'
			fprintf(stderr, "Xuan's Shell Error: Failed to obtain the path\n");
		} else {
			fprintf(stdout, "Xuan's Shell: Successfully obtain the work directory: %s\n", path);	
		}
		...
		if (chmod(argv[1], mod) < 0) { // 使用'chmod' 
			fprintf(stderr, "Xuan's Shell Error: Fail to change the mode of %s\n", argv[1]);
		} else {
			fprintf(stdout, "Xuan's Shell: Successfully change the file mode\n");	
		}
	} else { // 其他命令（默认为外部命令）
		pid_t pid = fork();
		if(pid < 0) {
			fprintf(stderr, "Xuan's Shell Error: Unsuccessful fork\n");
			exit(1);}
		if(pid == 0) {
			if (execvp(argv[0], argv) < 0) {
				fprintf(stderr, "Xuan's Shell Error: Unsupported Instruction!!!\n");
				exit(1);}
		}
		if (pid > 0) {
			wait(NULL);}
